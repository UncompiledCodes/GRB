...
autoSmoothX = False
smoothX = .4 # smooth of spline for X-ray
...
def readX(filename, cutoff = 1e8):
    ...
    luminErrNegs = []
    with open(filename) as fd:
        data = csv.DictReader(fd, delimiter = ' ', fieldnames=("time", "lumin", "timeErrPos", "timeErrNeg","luminErrPos", "luminErrNeg","PI","PIErrNeg","PIErrPos"))
...
def readO(filename, cutoff = 1e8):
    ...
    try:
        with open(filename) as fd:
            data = csv.DictReader(fd, delimiter = ',', fieldnames=("time", "lumin", "luminErrPos", "luminErrNeg","band"))
            # sort data by a given column 
            sortedData = sorted(data, key =lambda d: float(d['time']))
            for row in sortedData:
                if float(row['time']) < cutoff:
...
if (autoSmoothX): smoothX = np.log10(len(yX))
splX = UnivariateSpline(np.log10(xX), np.log10(yX))
splX.set_smoothing_factor(smoothX)
plt.errorbar(xX, yX, yerr=[yErrNegX,yErrPosX],fmt='.', elinewidth=0.5, markersize = 8, capsize = 0, label = 'X-ray: '+'0.3-10 KeV')
plt.loglog(10**np.log10(xX), 10**splX(np.log10(xX)), '--', color='steelblue', lw=0.5)
...
if (xO !=0 ):
    ...
    plt.errorbar(xO, yO, yerr=[yErrNegO,yErrPosO],fmt='^', elinewidth=0.5, markersize = 8, capsize = 0, label = 'Optical: '+bands[0]+' band')
...
plt.axvline(tPeaks[GRB],linewidth=0.5, linestyle = 'solid', color='r', alpha = 1, label = 'Peak Time: '+ str(tPeaks[GRB])+'s'+ "\n" + 'Duration: '+ str(deltaTs[GRB])+'s')
legend = plt.legend(loc=1, title = 'GRB '+ GRB +"\n"+"\n"+'Redshift:'+ str(redshifts[GRB])+"\n"+'Eiso:'+ str(energys[GRB])+'erg')
